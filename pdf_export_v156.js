...
.ctPdfRoot .pdfContent{height:277.5mm!important;max-height:277.5mm!important;overflow:hidden!important;position:relative!important;z-index:1!important;padding:0!important;margin:0!important;}
...
.ctPdfRoot .missedOk{margin-top:2.6mm!important;padding:1.8mm!important;font-size:9pt!important;border-radius:2.5mm!important;border:1px solid #cfe8d4!important;background:#f3faf5!important;color:#137333!important;font-weight:800!important;}
  ...
  function ensureStyle(){
    let st = document.getElementById('ctPdfRasterStyleV156');
    if(!st){
      st = document.createElement('style');
      st.id = 'ctPdfRasterStyleV156';
      st.textContent = v156Css();
  ...
  function buildMeasuredPages(data){
    ensureStyle();
    const old = document.getElementById('ctPdfMeasureRootV156');
    if(old) old.remove();
    const root = document.createElement('div');
    root.id = 'ctPdfMeasureRootV156';
  ...
  function installRasterPdfOverride(){
    function handler(e){
      const target = e.target && e.target.closest && e.target.closest('#exportPdfResult,#exportPdf');
      if(!target) return;
      e.preventDefault();
      e.stopPropagation();
    ...
    const bind = () => {
      ['exportPdfResult','exportPdf'].forEach(id=>{
        const b = document.getElementById(id);
        if(b) b.onclick = exportRasterPdf;
      });
    };
    bind();
