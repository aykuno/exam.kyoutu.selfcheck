(() => {
  ...
  const TABLE_COLUMNS = [190, 120, 310, 90, 145, 125, 144];
  ...
  function drawTableHeader(ctx, y, continued, pointsAvailable) {
    ...
    const headers = ["番号", "自分", "正解", "判定", pointsAvailable ? "得点" : "配点", "受験者正答率", "注記"];
    ctx.fillStyle = "#f5f7fb";
    ctx.fillRect(LEFT, y, CONTENT_W, 32);
    let x = LEFT;
    headers.forEach((header, index) => {
      setFont(ctx, 12, "800", COLORS.muted, index === 4 ? "center" : "left");
      ctx.fillText(header, index === 3 ? x + TABLE_COLUMNS[index] / 2 : x + 9, y + 8);
  ...
  function rowHeight(ctx, row) {
    ...
    const noteLines = wrapLines(ctx, row.note || "—", TABLE_COLUMNS[6] - 18, 2).length;
    return Math.max(34, 14 + Math.max(gotLines, expectedLines, noteLines) * 18);
  }
